...
app.post('/api/employees', async (req: Request, res: Response) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO employees (name, email, position, department, salary, hire_date) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
      [name, email, position, department, salary, hire_date || new Date().toISOString().split('T')[0]]
    );
